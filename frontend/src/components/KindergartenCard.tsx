  ...
  const feeText =
    kindergarten.fee_min || kindergarten.fee_max
      ? `${kindergarten.fee_currency ?? "NZD"} ${
          ...
          kindergarten.fee_max ? ` - ${kindergarten.fee_max.toLocaleString()}` : ""
        }${
  ...
  return (
    <article className="flex flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <header className="mb-2 flex items-start justify-between gap-2">
        <div>
          ...
          {kindergarten.brand_name && (
            <p className="text-[11px] text-slate-500">{kindergarten.brand_name}</p>
          )}
          {kindergarten.city && (
            <p className="text-[11px] text-slate-500">{kindergarten.city}</p>
          )}
        </div>
      ...
      <div className="space-y-1 text-xs text-slate-600">
        <p>{feeText}</p>
        {kindergarten.owner_or_group && (
          <p>
            <span className="font-medium">Owner / Group:</span> {kindergarten.owner_or_group}
          </p>
        )}
      </div>
    </article>
